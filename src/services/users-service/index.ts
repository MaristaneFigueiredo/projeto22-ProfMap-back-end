...
async function createUser({name, email, password}: CreateUserParams): Promise<users> {
    ...
    return await usersRepository.createUser({name, email, password: hashedPassword})
...
async function validateCredentialsAndGetToken({ email, password }: CreateAuthParams) {
